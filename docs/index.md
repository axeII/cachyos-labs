# CachyOS Tweaks Documentation

Welcome to the CachyOS Tweaks documentation. I like to optimize my system to max feel free to use tools I have in this repository same may be useful to you.

## Quick Start

- [README](../README.md) — Project overview and quick start
- [Installation Script](../install.sh) — Automated setup

## Topics

- [Power Management](power-management.md) — Auto-switching tuned profiles
- [Display & Audio](display-audio.md) — Multi-monitor and HDMI audio
- [GPU Tuning](gpu-tuning.md) — RX 9070 XT configuration
- [CPU & Cooling](cpu-cooling.md) — Fan control and CPU optimizations
- [Gaming](gaming.md) — MangoHud, Proton, Steam tips
- [Game Tools](game-tools.md) — OptiScaler manager
- [Desktop](desktop.md) — WezTerm, Clonky, notifications

## Reference

- [CHANGELOG](../CHANGELOG.md) — Full history of changes
- [Active vs Tested](../ACTIVE-vs-TEST.md) — What's running vs. what was tried
